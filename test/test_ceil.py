...
def test_ceil_all_dtypes():
    ...
    for ms_dtype, torch_dtype in zip(mindspore_dtypes, torch_dtypes):
        ...
        try:
            # 调用 ceil 函数
            torch_output = torch.ceil(torch_input)
            # 对比结果shape
...
'''
def test_ceil_fixed_dtype_random_input():

    # 固定dtype为float16、float32、float64、bfloat16
    dtypes = [mindspore.float16, mindspore.float32, mindspore.float64, mindspore.bfloat16]
...
'''
def test_ceil_different_input_types():
    # 固定 shape 为 (3,)
    shape = (3,)
    # 定义不同的输入类型
    ...
    for input_name, input_value in input_types.items():
        try:
            # 测试 MindSpore
            ms_output = mint.ceil(input_value)
            # shape 与预期一致
        ...
        try:
            # 测试 PyTorch
            torch_output = torch.ceil(input_value)
            # shape 与预期一致
...
d) 测试随机混乱输入，报错信息的准确性
'''
def test_ceil_random_chaotic_input():
    # 定义随机混乱输入
    chaotic_inputs = [
        ...
        mindspore.tensor(np.array([1.1, 2.5, -1.5]), dtype=mindspore.float32),  # MindSpore tensor 浮点数
        mindspore.tensor(np.array([[1, 2, 3], [4, 5, 6]]), dtype=mindspore.int32),  # MindSpore tensor 整型
    ...
    for input_data in chaotic_inputs:
        try:
            ms_output = mint.ceil(input_data)
        ...
        try:
            torch_output = torch.ceil(input_data)
...
def test_forward_backward():
    class SimpleModel(nn.Module):
        ...
        def forward(self, x):
            return torch.ceil(self.linear(x))
    ...
    class SimpleModelMS(mnn.Cell):
        ...
        def construct(self, x):
            return mint.ceil(self.linear(x))
...
# 基础测试函数
# test_ceil_all_dtypes()
# test_ceil_fixed_dtype_random_input()
# test_ceil_different_input_types()
# test_ceil_random_chaotic_input()
