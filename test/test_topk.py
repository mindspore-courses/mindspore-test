...
def is_same(input_data_x=[[1, 2], [3, 4], [5, 6]], input_data_y=[[6, 5], [4, 3], [2, 1]], shape=Notopk, dtype_ms=ms.float32, dtype_torch=torch.float32):
    ...
    ms_result = mint.topk(ms_tensor_x, ms_tensor_y).asnumpy()
    torch_result = torch.topk(torch_tensor_x, torch_tensor_y).numpy()
...
def test_topk_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
        ...
        try:
            ...
            err = True
            print(f"mint.topk not supported for {dtype_ms}")

        try:
            torch_result = torch.topk(torch_tensor_x, torch_tensor_y).numpy()
        except Exception as e:
            err = True
            print(f"torch.topk not supported for {dtype_torch}")

        if not err:
            if not np.allclose(ms_result, torch_result):
                print(f"mint.topk is supported for {dtype_ms} but not working properly")


@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_topk_precision_alignment(mode):
    """验证不同维度随机输入的精度对齐"""
    ms.set_context(mode=mode)
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_topk_wrong_input(mode):
    """测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)
    ...
    # 测试非法输入类型（字符串代替张量）
    try:
        ms.mint.topk("invalid_input", valid_tensor)
    except TypeError as e:
        assert "input" in str(e) and "Tensor" in str(e), f"错误信息不准确，实际信息：{e}"
        print(f"[topk] TypeError 测试通过！非法输入类型检测成功，报错信息：\n{e}")

    # 测试非法的数据类型（字典类型）
    try:
        ms.mint.topk({'a': 1}, valid_tensor)
    except TypeError as e:
        assert "utopkxpected keyword argument" not in str(e), "应捕获输入类型错误而非参数名错误"
        print(f"[topk] TypeError 测试通过！非法数据类型检测成功，报错信息：\n{e}")
    ...
    try:
        ...
        print(f"[topk] ValueError 测试通过！形状不匹配检测成功，报错信息：\n{e}")

    # 测试无效关键字参数
    try:
        ms.mint.topk(valid_tensor, valid_tensor, nonsuch_param=1)
    except TypeError as e:
        assert "nonsuch_param" in str(e), "应明确提示无效参数名"
        print(f"[topk] TypeError 测试通过！无效参数检测成功，报错信息：\n{e}")
    ...
    try:
        ...
        print(f"[topk] TypeError 测试通过！双布尔类型检测成功，报错信息：\n{e}")

    # 对比PyTorch的报错行为
    try:
        torch.topk(torch_valid, "invalid_type")
    except TypeError as e:
        assert "Tensor" in str(e), "PyTorch应拒绝非Tensor输入"
        print(f"[PyTorch topk] TypeError 一致性验证通过，报错信息：\n{e}")
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_topk_forward_back(mode):
    ...
    def forward_pt(x, y):
        # 将比较结果作为掩码，与可导操作结合
        mask = torch.topk(x, y).float()  # 布尔转浮点
        return (x * mask).sum()  # 可导路径：x * mask

    def forward_ms(x, y):
        mask = mint.topk(x, y).astype(ms.float32)  # 使用astype正确转换类型
        return (x * mask).sum()
...
def adapt_data(dtype):
    """适配数据类型生成合规数据"""
    data = np.array(input_data)
    if dtype in [ms.uint8, ms.uint16, ms.uint32, ms.uint64]:
        return np.abs(data)
    if dtype == ms.bool_:
        return data % 2  # 布尔类型生成0/1值
    return data


@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])  # 保留参数化装饰器
def test_topk_different_dtypes(mode):
    """测试random输入不同dtype，对比两个框架的支持度"""
    ms.set_context(mode=mode)
    # 原始数据类型列表保留
    ms_dtypes = [ms.int8, ms.int16, ms.int32, ms.int64, ms.uint8, ms.uint16, ms.uint32, ms.uint64, ms.float16,
    ...
    for i in range(len(ms_dtypes)):
        ...
        # 数据适配
        np_data = adapt_data(dtype_ms)

        try:
            ms_tensor = Tensor(np_data).astype(dtype_ms)
            torch_tensor = torch.tensor(np_data).type(dtype_torch)
        except Exception as e:
            print(f"Tensor创建失败: {dtype_ms} | 错误: {str(e)}")
            continue

        err = False
        try:
            ms_val, ms_idx = mint.topk(ms_tensor, k=2, dim=1)
            ms_val = ms_val.asnumpy()
            ms_idx = ms_idx.asnumpy()
        except Exception as e:
            err = True
            print(f"mint.topk不支持: {dtype_ms}")

        try:
            torch_val, torch_idx = torch.topk(torch_tensor, k=2, dim=1)
            torch_val = torch_val.numpy()
            torch_idx = torch_idx.numpy()
        except Exception as e:
            err = True
            print(f"torch.topk不支持: {dtype_torch}")

        if not err:
            assert np.allclose(ms_val, torch_val), f"数值不匹配 {dtype_ms}"
            assert np.array_equal(ms_idx, torch_idx), f"索引不匹配 {dtype_ms}"