...
def check(arr1, arr2, threshold=1e-3):
    ...
    if np.any(diff >= threshold):
        ...
        # print("arr1:",arr1)
        # print("arr2:",arr2)
        # error_positions = np.argwhere(diff >= threshold)
        # print(f"误差大于阈值 {threshold} 的位置:")
        # for pos in error_positions:
            # index_tuple = tuple(pos)
            # print(f"{diff[index_tuple]}")
        return False,diff
...
def test_any_random_input_fixed_dtype(mode):
    ...
    for type in types:
        for _ in range(100):
          ...
          ms_res = ms.mint.atan2(ms_inputs,ms_others)
          torch_res=torch.atan2(torch_inputs,torch_others)
          #转为numpy数组进行比较
...
def test_any_different_para(mode):
    ...
    assert True#由于atan2只有input、other参数，因此不再重复test_any_different_dtypes的测试
...
def test_any_wrong_input(mode):
    ...
    #4.4非法数据结构（非torch或常数）
    inputs=['123',[0.1,-0.4],{'a':1,'b':2},0.1,True,np.array([0.1,-0.3]),None,{'a':1,'b':2},np.array([0.1,-0.3]),None]
    others=['123',[0.1,-0.4],{'a':1,'b':2},0.1,True,'123',np.array([0.1,-0.3]),np.array([0.1,-0.3]),None,None]
    print("\n输入非法数据结构：")
    for i,inp in enumerate(inputs):
      ...
      if type(oth) is np.ndarray:
         ...
         torch_oth=oth
      try:
        print(torch.atan2(torch_inp,torch_oth))  
      except Exception as e:
        print(e)
      print("MindSpore：",end="")
      if type(inp) is np.ndarray:
      ...
      if type(oth) is np.ndarray:
         ...
         ms_oth=oth
      try:
        print(ms.mint.atan2(ms_inp,ms_oth))  
      except Exception as e:
        print(e)
