...
def test_selu_dtypes_support(mode):
    """测试不同dtype输入的支持情况"""
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_selu_fixed_dtype_accuracy(mode):
    """固定dtype测试输出一致性"""
    ms.set_context(mode=mode, device_target='Ascend')
    ...
    out_torch = F_torch.selu(x_torch)
    
    # MindSpore
    x_ms = Tensor(x_np, ms.float32)
    out_ms = F_mint.selu(x_ms)
    
    assert np.allclose(out_ms.asnumpy(), out_torch.detach().numpy(), atol=1e-3), "输出不一致"

@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_selu_invalid_input(mode):
    ms.set_context(mode=mode, device_target='Ascend')
    """测试随机混乱输入，报错信息的准确性"""
    
    # 测试非Tensor输入
    with pytest.raises(TypeError):
        F_mint.selu(np.random.randn(2,3))

class TorchSeLUNet(torch.nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        return F_torch.selu(self.linear(x))

class MsSeLUNet(ms.nn.Cell):
    def __init__(self):
        super().__init__()
    ...
    def construct(self, x):
        return F_mint.selu(self.linear(x))

@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_selu_network_forward(mode):
    """测试网络正向传播一致性"""
    ms.set_context(mode=mode, device_target='Ascend')
    x_np = np.random.randn(2, 3).astype(np.float32)
    
    # PyTorch
    torch_net = TorchSeLUNet()
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_selu_network_backward(mode):
    """测试网络反向传播梯度"""
    ms.set_context(mode=mode, device_target='Ascend')
    x_np = np.random.randn(2, 3).astype(np.float32)
    y_np = np.array([0, 1])
