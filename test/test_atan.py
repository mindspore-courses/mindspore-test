...
def test_atan_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
        ...
        err = False
        try:
            ms_result = mint.atan(ms_input).asnumpy()
        except Exception as e:
            err = True
            print(f"mint.atan not supported for {dtype_ms}")
        try:
            torch_result = torch.atan(torch_input).numpy()
        except Exception as e:
            err = True
            print(f"torch.atan not supported for {dtype_torch}")
        if not err:
            try:
...
@pytest.mark.parametrize("mode", [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_atan_fixed_dtype_random_input(mode):
...
@pytest.mark.parametrize("mode", [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_atan_fixed_shape_input_diff_param(mode):
    ...
    torch_input = torch.tensor(input_data, dtype=torch.float32)
    try:
        ms_result = mint.atan(ms_input, "extra_param").asnumpy()
    except Exception as e:
        print(f"mint.atan extra param error:", e)
    try:
        torch_result = torch.atan(torch_input, "extra_param").numpy()
    except Exception as e:
        print(f"torch.atan extra param error:", e)


@pytest.mark.parametrize("mode", [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_atan_random_input_error_message(mode):
    """1.(d) 测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)
    input_data = np.array([0, -0.5])
    ms_input = Tensor(input_data, ms.float32)
    try:
        ms_result = mint.atan(1.5).asnumpy()
    except Exception as e:
        print(f"mint.atan error message:", e)
    try:
        ms_result = mint.atan(True).asnumpy()
    except Exception as e:
        print(f"mint.atan error message:", e)
    try:
        ms_result = mint.atan(ms_input, "extra_param").asnumpy()
