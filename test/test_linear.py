...
def test_linear_all_dtypes(mode):
    ...
    for ms_dtype, torch_dtype in zip(ms_dtypes, torch_dtypes):
        ...
        try:
            # 调用 linear 函数
            torch_output = torch.nn.functional.linear(torch_input, torch_weight)
            # 对比结果shape
            assert torch_output.shape == (5, 2)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_linear_fixed_dtype_random_input(mode):
    ms.set_context(mode=mode)
    # 固定dtype为float16、float32、bfloat16
    dtypes = [ms.float16, ms.float32, ms.bfloat16]
    torch_dtypes = [torch.float16, torch.float32, torch.bfloat16]
    ...
    for dtype, torch_dtype in zip(dtypes, torch_dtypes):
        for _ in range(100):
            ...
            # 生成权重和偏置
            weight_data = np.random.randn(2, 3).astype(np.float32)
            ms_weight = ms.tensor(weight_data, dtype)
            torch_weight = torch.tensor(weight_data, dtype=torch_dtype)

            bias_data = np.random.randn(2).astype(np.float32)
            ms_bias = ms.tensor(bias_data, dtype)
            torch_bias = torch.tensor(bias_data, dtype=torch_dtype)

            # 计算输出
            ms_output = mint.nn.functional.linear(ms_input, ms_weight, ms_bias)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_linear_different_input_types(mode):
    ms.set_context(mode=mode)
    # 固定 shape 为 (3,2)
    shape = (3, 2)
    input_data = [1.1, 2.5, -1.5]
    input_types = {
    ...
    for input_name, input_value in input_types.items():
        try:
            # 测试 MindSpore
            ms_output = mint.nn.functional.linear(input_value, ms.tensor(np.random.randn(3, 2), ms.float32), ms.tensor(np.random.randn(2), ms.float32))
            # shape 与预期一致
        ...
        try:
            # 测试 PyTorch
            torch_output = torch.nn.functional.linear(input_value, torch.tensor(np.random.randn(3, 2), dtype=torch.float32), torch.tensor(np.random.randn(2), dtype=torch.float32))
            # shape 与预期一致
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_linear_random_chaotic_input(mode):
    ms.set_context(mode=mode)
    # 定义随机混乱输入
    chaotic_inputs = [
    ...
    for input_data in chaotic_inputs:
        try:
            ms_output = mint.nn.functional.linear(input_data, ms.tensor(np.random.randn(3, 2), ms.float32), ms.tensor(np.random.randn(2), ms.float32))
        ...
        try:
            torch_output = torch.nn.functional.linear(input_data, torch.tensor(np.random.randn(3, 2), dtype=torch.float32), torch.tensor(np.random.randn(2), dtype=torch.float32))
...
def test_forward_backward(mode):
    ...
    class SimpleModel(nn.Module):
        def __init__(self):
            ...
            self.linear1 = nn.Linear(3, 3)  # 输入维度为3，输出维度为3
            self.linear2 = nn.Linear(3, 2)  # 输入维度为3，输出维度为2

        def forward(self, x):
            x = torch.nn.functional.linear(x, self.linear1.weight, self.linear1.bias)
            x = torch.nn.functional.linear(x, self.linear2.weight, self.linear2.bias)
            return x

    # Simple MindSpore model
    class SimpleModelMS(mnn.Cell):
        def __init__(self):
            super(SimpleModelMS, self).__init__()
            self.linear1 = mnn.Dense(3, 3)  # 输入维度为3，输出维度为3
            self.linear2 = mnn.Dense(3, 2)  # 输入维度为3，输出维度为2

        def construct(self, x):
            x = mint.nn.functional.linear(x, self.linear1.weight, self.linear1.bias)
            x = mint.nn.functional.linear(x, self.linear2.weight, self.linear2.bias)
            return x

    # 创建固定输入和权重
    input_data_np = np.array([[1.5, -2.3, 3.6], [-1.2, 0.4, -0.9]], dtype=np.float32)
    weight_np1 = np.random.randn(3, 3).astype(np.float32)
    weight_np2 = np.random.randn(2, 3).astype(np.float32)
    bias_np1 = np.random.randn(3).astype(np.float32)
    bias_np2 = np.random.randn(2).astype(np.float32)

    # PyTorch模型设置
    model_torch = SimpleModel()
    model_torch.linear1.weight.data = torch.tensor(weight_np1)  
    model_torch.linear1.bias.data = torch.tensor(bias_np1)
    model_torch.linear2.weight.data = torch.tensor(weight_np2)
    model_torch.linear2.bias.data = torch.tensor(bias_np2)
    input_torch = torch.tensor(input_data_np, requires_grad=True)

    # MindSpore模型设置
    model_ms = SimpleModelMS()
    model_ms.linear1.weight.set_data(Tensor(weight_np1))  # 不需要转置
    model_ms.linear1.bias.set_data(Tensor(bias_np1))
    model_ms.linear2.weight.set_data(Tensor(weight_np2))
    model_ms.linear2.bias.set_data(Tensor(bias_np2))
