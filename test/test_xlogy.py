...
def test_xlogy_all_dtypes(mode):
    ...
    for ms_dtype, torch_dtype in zip(ms_dtypes, torch_dtypes):
        ...
        try:
            # 调用 xlogy 函数
            ms_output = mint.xlogy(ms_input,ms.tensor([2.0] * len(input_data),ms_dtype))
            # 对比结果shape
        ...
        try:
            # 调用 xlogy 函数
            torch_output = torch.xlogy(torch_input,torch.tensor([2.0] * len(input_data)))
            # 对比结果shape
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_xlogy_fixed_dtype_random_input(mode):
    ms.set_context(mode=mode)
    ...
    for dtype, torch_dtype in zip(dtypes, torch_dtypes):
        for _ in range(100):
            ...
            # 计算输出
            ms_output = mint.xlogy(ms_input,ms.tensor([2.0] * len(input_data),dtype))
            torch_output = torch.xlogy(torch_input,torch.tensor([2.0] * len(input_data),dtype=torch_dtype))
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_xlogy_different_input_types(mode):
    ms.set_context(mode=mode)
    # 固定 shape 为 (3,)
    shape = (3,)
    # 定义不同的输入类型
    ...
    for input_name, input_value in input_types.items():
        try:
            # 测试 MindSpore
            ms_output = mint.xlogy(input_value,ms.tensor([2.0] * len(input_data),ms_dtype))
            # shape 与预期一致
        ...
        try:
            # 测试 PyTorch
            torch_output = torch.xlogy(input_value,torch.tensor([2.0] * len(input_data)))
            # shape 与预期一致
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_xlogy_random_chaotic_input(mode):
    ms.set_context(mode=mode)
    # 定义随机混乱输入
    chaotic_inputs = [
    ...
    for input_data in chaotic_inputs:
        try:
            ms_output = mint.xlogy(input_data,ms.tensor([2.0] * len(input_data),ms_dtype))
        ...
        try:
            torch_output = torch.xlogy(input_data,torch.tensor([2.0] * len(input_data)))
...
def test_forward_backward(mode):
    ...
    class SimpleModel(nn.Module):
        ...
        def forward(self, x):
            return torch.xlogy(self.linear(x),torch.tensor([2.0] * 3))
    ...
    class SimpleModelMS(mnn.Cell):
        ...
        def construct(self, x):
            return  mint.xlogy(self.linear(x),ms.tensor([2.0] * 3))
