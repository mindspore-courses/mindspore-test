...
def test_SELU_all_dtypes(mode):
    ...
    for ms_dtype, torch_dtype in zip(ms_dtypes, torch_dtypes):
        ...
        try:
            # 调用 SELU 函数
            ms_SELU = mint.nn.SELU()
            ms_output = ms_SELU(ms_input)
            # 对比结果shape
        ...
        try:
            ...
            torch_output = torch_SELU(torch_input)
            # 对比结果shape
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_SELU_fixed_dtype_random_input(mode):
    ms.set_context(mode=mode)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_SELU_different_input_types(mode):
    ms.set_context(mode=mode)
    # 固定 shape 为 (3,)
    shape = (3,)
    # 定义不同的输入类型
    ...
    for input_name, input_value in input_types.items():
        ...
        try:
            # 测试 PyTorch
            torch_SELU = torch.nn.SELU()
            torch_output = torch_SELU(input_value)
            # shape 与预期一致
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_SELU_random_chaotic_input(mode):
    ms.set_context(mode=mode)
    # 定义随机混乱输入
    chaotic_inputs = [
    ...
    for input_data in chaotic_inputs:
        ...
        try:
            torch_SELU = torch.nn.SELU()
            torch_output = torch_SELU(input_data)
...
def test_forward_backward(mode):
    ...
    class SimpleModel(nn.Module):
        ...
        def forward(self, x):
            torch_SELU = torch.nn.SELU()
            return torch_SELU(self.linear(x))
