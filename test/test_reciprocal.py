...
def test_reciprocal_all_dtypes(mode):
    ...
    for ms_dtype, torch_dtype in zip(ms_dtypes, torch_dtypes):
        ...
        try:
            # 调用 reciprocal 函数
            ms_output = mint.reciprocal(ms_input)
            # 对比结果shape
        ...
        try:
            # 调用 reciprocal 函数
            torch_output = torch.reciprocal(torch_input)
            # 对比结果shape
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_reciprocal_fixed_dtype_random_input(mode):
    ms.set_context(mode=mode)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_reciprocal_different_input_types(mode):
    ms.set_context(mode=mode)
    # 固定 shape 为 (3,)
    shape = (3,)
    # 定义不同的输入类型
    ...
    for input_name, input_value in input_types.items():
        try:
            # 测试 MindSpore
            ms_output = mint.reciprocal(input_value)
            # shape 与预期一致
        ...
        try:
            # 测试 PyTorch
            torch_output = torch.reciprocal(input_value)
            # shape 与预期一致
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_reciprocal_random_chaotic_input(mode):
    ms.set_context(mode=mode)
    # 定义随机混乱输入
    chaotic_inputs = [
    ...
    for input_data in chaotic_inputs:
        try:
            ms_output = mint.reciprocal(input_data)
        ...
        try:
            torch_output = torch.reciprocal(input_data)
...
def test_forward_backward(mode):
    ...
    class SimpleModel(nn.Module):
        ...
        def forward(self, x):
            return torch.reciprocal(self.linear(x))
    ...
    class SimpleModelMS(mnn.Cell):
        ...
        def construct(self, x):
            return  mint.reciprocal(self.linear(x))
