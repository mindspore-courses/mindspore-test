...
def test_max_pool2d_all_dtypes(mode):
    ...
    for ms_dtype, torch_dtype in zip(ms_dtypes, torch_dtypes):
        ...
        try:
            # 调用 max_pool2d 函数
            ms_output = mint.nn.functional.max_pool2d(ms_input, kernel_size=2)
        ...
        try:
            # 调用 max_pool2d 函数
            torch_output = torch.nn.functional.max_pool2d(torch_input, kernel_size=2)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_max_pool2d_fixed_dtype_random_input(mode):
    ms.set_context(mode=mode)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_max_pool2d_different_input_types(mode):
    ms.set_context(mode=mode)
    # 固定 shape 为 (1, 3, 8, 8)
    shape = (1, 3, 8, 8)
    # 定义不同的输入类型
    ...
    for input_name, input_value in input_types.items():
        try:
            # 测试 MindSpore
            ms_output = mint.nn.functional.max_pool2d(input_value, kernel_size=2)
        ...
        try:
            # 测试 PyTorch
            torch_output = torch.nn.functional.max_pool2d(input_value, kernel_size=2)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_max_pool2d_random_chaotic_input(mode):
    ms.set_context(mode=mode)
    # 定义随机混乱输入
    chaotic_inputs = [
    ...
    for input_data in chaotic_inputs:
        try:
            ms_output = mint.nn.functional.max_pool2d(input_data, kernel_size=2)
        ...
        try:
            torch_output = torch.nn.functional.max_pool2d(input_data, kernel_size=2)
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_max_pool2d_forward_backward(mode):
    ms.set_context(mode=mode)
    ...
    class SimpleModelMS(mnn.Cell):
        def __init__(self):
            ...
            self.conv = mnn.Conv2d(1, 1, kernel_size=3, stride=1, pad_mode="pad", padding=1)
            self.pool = mnn.MaxPool2d(kernel_size=2, stride=2)
    ...
    # 创建固定输入和权重
    input_data_np = np.random.randn(1, 1, 8, 8).astype(np.float32)
    weight_np = np.random.randn(1, 1, 3, 3).astype(np.float32)
    bias_np = np.random.randn(1).astype(np.float32)

    # PyTorch模型设置
    model_torch = SimpleModel()
    model_torch.conv.weight.data = torch.tensor(weight_np)  
    model_torch.conv.bias.data = torch.tensor(bias_np)
    ...
    # 初始化权重和偏置
    model_ms.conv.weight = Tensor(weight_np)  # 初始化卷积层的权重
    model_ms.conv.bias = Tensor(bias_np)  # 初始化卷积层的偏置
    ...
    # 比较反向传播结果（输入梯度）
    np.testing.assert_allclose(grad_input_ms.asnumpy(), grad_input_torch.detach().numpy(), rtol=1e-3)
