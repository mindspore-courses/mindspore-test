...
def is_same(input_data=[[1,6,2,4],[7,3,8,2],[2, 9, 11, 5]], shape=None, dtype_ms=ms.float32, dtype_torch=torch.float32, dim=-1, keepdim=False):
    ...
    if dim == None:
        ms_result = mint.min(ms_tensor)
        torch_result = torch.min(torch_tensor)
        return np.allclose(ms_result.asnumpy(), torch_result.numpy())
    else:
        ms_result, ms_index = mint.min(ms_tensor, dim=dim, keepdim=keepdim)
        torch_result, torch_index = torch.min(torch_tensor, dim=dim, keepdim=keepdim)
        return np.allclose(ms_result.asnumpy(), torch_result.numpy()) and np.allclose(ms_index.asnumpy(), torch_index.numpy()) 
    

@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_min_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
        ...
        err = False
        try:
            ms_result = mint.min(ms_tensor).asnumpy()
        ...
        try:
            torch_result = torch.min(torch_tensor).numpy() 
        except Exception as e:
            err = True
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_min_random_input_fixed_dtype(mode):
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_min_different_para(mode):
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_min_wrong_input(mode):
    """测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)

    try:
        ms_result = mint.min(ms_tensor, dim=None, keepdim=True).asnumpy()
    except Exception as e:
        print(f"dim 为 None 且 keepdim 不是 False报错信息：\n{e}")


@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_min_forward_back(mode):
    ...
    def forward_ms(x):
        return mint.min(x, dim=1, keepdim=False)
    
    grad_fn = value_and_grad(forward_ms)
    output_ms, gradient_ms = grad_fn(ms_tensor)
    
    output_pt = forward_pt(torch_tensor)
    for value in output_pt.values:
        value.backward(retain_graph=True)
    gradient_pt = torch_tensor.grad

    # print(output_ms, output_pt)
    # print(gradient_ms, gradient_pt)
    assert np.allclose(output_ms[0].asnumpy(), output_pt.values.detach().numpy(), atol=1e-3)
    assert np.allclose(output_ms[1].asnumpy(), output_pt.indices.detach().numpy(), atol=1e-3)
