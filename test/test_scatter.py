...
def perform_scatter(
    ...
    ms_result = mint.scatter(ms_tensor, dim, ms_index, ms_src)
    torch_result = torch.scatter(torch_tensor, dim, torch_index, torch_src)
...
def test_scatter_different_dtypes(mode, dtype_pair):
    ...
    try:
        ms_result, torch_result = perform_scatter(
            ms_tensor,
            torch_tensor,
            ms_src,
            torch_src,
...
def test_scatter_random_input_fixed_dtype(mode, input_data, src_data):
...
def test_scatter_different_para(mode, dtype_pair, dim):
...
def test_scatter_wrong_input(mode, input_data, src_data, index):
    """测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)
...
@pytest.mark.parametrize("mode", [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_scatter_forward_back(mode):
    """测试前向和反向传播，对比梯度"""
    ms.set_context(mode=mode)

    def forward_ms(x, src, idx):
        return mint.scatter(x, 0, idx, src).sum()

    def forward_torch(x, src, idx):
        return torch.scatter(x, 0, idx, src).sum()
