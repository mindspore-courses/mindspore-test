...
def test_roll_all_dtypes(mode):
    ...
    for ms_dtype, torch_dtype in zip(ms_dtypes, torch_dtypes):
        ...
        try:
            # 调用 roll 函数
            ms_output = mint.roll(ms_input,shifts=0)
            # 对比结果shape
        ...
        try:
            ...
            torch_output = torch.roll(torch_input,shifts=0)
            # 对比结果shape
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_roll_fixed_dtype_random_input(mode):
    ms.set_context(mode=mode)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_roll_different_input_types(mode):
    ms.set_context(mode=mode)
    # 固定 shape 为 (3,)
    shape = (3,)
    # 定义不同的输入类型
    ...
    for input_name, input_value in input_types.items():
        try:
            ...
            ms_output = mint.roll(input_value,shifts=0)
            # shape 与预期一致
        ...
        try:
            ...
            torch_output = torch.roll(inputvalue,shifts=0)
            # shape 与预期一致
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_roll_random_chaotic_input(mode):
    ms.set_context(mode=mode)
    # 定义随机混乱输入
    chaotic_inputs = [
    ...
    for input_data in chaotic_inputs:
        try:
            ...
            ms_output = mint.roll(input_data,shifts=0)
        ...
        try:
            ...
            torch_output = torch.roll(input_data,shifts=0)
...
def test_forward_backward(mode):
    ...
    class SimpleModel(nn.Module):
        ...
        def forward(self, x):
            ...
            return torch.roll(self.linear(x),shifts=0)
    ...
    class SimpleModelMS(mnn.Cell):
        ...
        def construct(self, x):
            ...
            return mint.roll(self.linear(x),shifts=0)
