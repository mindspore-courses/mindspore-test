...
def is_same(input_data_x=[[1, 2], [3, 4], [5, 6]], input_data_y=[[6, 5], [4, 3], [2, 1]], shape=None, dtype_ms=ms.float32, dtype_torch=torch.float32):
    ...
    ms_result = mint.maximum(ms_tensor_x, ms_tensor_y).asnumpy()
    torch_result = torch.maximum(torch_tensor_x, torch_tensor_y).numpy()
...
def test_maximum_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
        ...
        try:
            ...
            err = True
            print(f"mint.maximum not supported for {dtype_ms}")

        try:
            torch_result = torch.maximum(torch_tensor_x, torch_tensor_y).numpy()
        except Exception as e:
            err = True
            print(f"torch.maximum not supported for {dtype_torch}")

        if not err:
            if not np.allclose(ms_result, torch_result):
                print(f"mint.maximum is supported for {dtype_ms} but not working properly")


@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_maximum_precision_alignment(mode):
    """验证不同维度随机输入的精度对齐"""
    ms.set_context(mode=mode)
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_maximum_wrong_input(mode):
    """测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)
    ...
    # 测试非法输入类型（字符串代替张量）
    try:
        ms.mint.maximum("invalid_input", valid_tensor)
    except TypeError as e:
        assert "input" in str(e) and "Tensor" in str(e), f"错误信息不准确，实际信息：{e}"
        print(f"[maximum] TypeError 测试通过！非法输入类型检测成功，报错信息：\n{e}")

    # 测试非法的数据类型（字典类型）
    try:
        ms.mint.maximum({'a': 1}, valid_tensor)
    except TypeError as e:
        assert "unexpected keyword argument" not in str(e), "应捕获输入类型错误而非参数名错误"
        print(f"[maximum] TypeError 测试通过！非法数据类型检测成功，报错信息：\n{e}")
    ...
    try:
        ...
        print(f"[maximum] ValueError 测试通过！形状不匹配检测成功，报错信息：\n{e}")

    # 测试无效关键字参数
    try:
        ms.mint.maximum(valid_tensor, valid_tensor, nonsuch_param=1)
    except TypeError as e:
        assert "nonsuch_param" in str(e), "应明确提示无效参数名"
        print(f"[maximum] TypeError 测试通过！无效参数检测成功，报错信息：\n{e}")
    ...
    try:
        ...
        print(f"[maximum] TypeError 测试通过！双布尔类型检测成功，报错信息：\n{e}")

    # 对比PyTorch的报错行为
    try:
        torch.maximum(torch_valid, "invalid_type")
    except TypeError as e:
        assert "Tensor" in str(e), "PyTorch应拒绝非Tensor输入"
        print(f"[PyTorch maximum] TypeError 一致性验证通过，报错信息：\n{e}")
...
def test_framework_support(mode):
    ...
    for case in broadcast_cases:
        ...
        y = Tensor(case[1])
        try:
            out = mint.maximum(x, y)
            print(f"广播模式 {x.shape} vs {y.shape} 支持")
        except Exception as e:
            print(f"广播失败: {str(e)}")

@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_maximum_forward_back(mode):
