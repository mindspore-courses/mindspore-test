...
def is_same(input_data=[[1, 6, 2, 4], [7, 3, 8, 2], [2, 9, 11, 5]], shape=None, dtype_ms=ms.float32, dtype_torch=torch.float32):
    ...
    ms_result = mint.erf(ms_tensor).asnumpy()
    torch_result = torch.erf(torch_tensor).numpy()
...
def test_erf_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
        ...
        try:
            ...
            err = True
            print(f"mint.erf not supported for {dtype_ms}")

        try:
            torch_result = torch.erf(torch_tensor).numpy()
        except Exception as e:
            err = True
            print(f"torch.erf not supported for {dtype_torch}")

        if not err:
            if not np.allclose(ms_result, torch_result):
                print(f"mint.erf is supported for {dtype_ms} but not working properly")


@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_erf_random_input_fixed_dtype(mode):
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_erf_different_para(mode):
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_erf_wrong_input(mode):
    """测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)

    try:
        ms_result = mint.erf('a').asnumpy()
    except TypeError as e:
        print(f"测试通过！TypeError -input 不是 Tensor 报错信息：\n{e}")

    try:
        ms_result = mint.erf(Tensor(input_data, ms.int32)).asnumpy()
    except TypeError as e:
        print(f"测试通过！Ascend: 如果 input 的数据类型不是 float16、float32、float64、int64、bool、bfloat16 报错信息：\n{e}")


@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_erf_forward_back(mode):
