...
def test_group_norm_all_dtypes(mode):
    ...
    for ms_dtype, torch_dtype in zip(ms_dtypes, torch_dtypes):
        ...
        try:
            # 调用 group_norm 函数
            ms_output = mint.nn.functional.group_norm(ms_input, 3)
        ...
        try:
            # 调用 group_norm 函数
            torch_output = torch.nn.functional.group_norm(torch_input, 3)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_group_norm_fixed_dtype_random_input(mode):
    ms.set_context(mode=mode)
    ...
    for dtype, torch_dtype in zip(dtypes, torch_dtypes):
        for _ in range(100):
            ...
            # 对比输出
            np.testing.assert_allclose(ms_output.asnumpy(), torch_output.detach().numpy(), atol=1e-5)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_group_norm_different_input_types(mode):
    ms.set_context(mode=mode)
    ...
    for input_name, input_value in input_types.items():
        try:
            # 测试 MindSpore
            ms_output = mint.nn.functional.group_norm(input_value, 3)
        ...
        try:
            # 测试 PyTorch
            torch_output = torch.nn.functional.group_norm(input_value, 3)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_group_norm_random_chaotic_input(mode):
    ms.set_context(mode=mode)
    # 定义随机混乱输入
    chaotic_inputs = [
    ...
    for input_data in chaotic_inputs:
        try:
            ms_output = mint.nn.functional.group_norm(input_data, 3)
        ...
        try:
            torch_output = torch.nn.functional.group_norm(input_data, 3)
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_group_norm_forward_backward(mode):
    ms.set_context(mode=mode)
    ...
    class SimpleModel(torch.nn.Module):
        ...
        def forward(self, x):
            x = torch.nn.functional.group_norm(x, 3)
            return x
    ...
    class SimpleModelMS(mnn.Cell):
        ...
        def construct(self, x):
            x = mint.nn.functional.group_norm(x, 3)
            return x
    ...
    # 比较正向传播结果
    np.testing.assert_allclose(output_ms.asnumpy(), output_torch.detach().numpy(), atol=1e-5)
    ...
    # 比较反向传播结果（输入梯度）
    np.testing.assert_allclose(grad_input_ms.asnumpy(), grad_input_torch, atol=1e-5)
