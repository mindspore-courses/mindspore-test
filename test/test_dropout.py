...
def test_dropout_all_dtypes(mode):
    ...
    for ms_dtype, torch_dtype in zip(ms_dtypes, torch_dtypes):
        ...
        try:
            # 调用 dropout 函数
            ms_output = mint.nn.functional.dropout(ms_input)
            # 对比结果shape
        ...
        try:
            # 调用 dropout 函数
            torch_output = torch.nn.functional.dropout(torch_input)
            # 对比结果shape
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_dropout_fixed_dtype_random_input(mode):
    ms.set_context(mode=mode)
    # 固定dtype为float16、float32、float64、bfloat16
    dtypes = [ms.float16, ms.float32, ms.float64, ms.bfloat16]
    torch_dtypes = [torch.float16, torch.float32, torch.float64, torch.bfloat16]
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_dropout_different_input_types(mode):
    ms.set_context(mode=mode)
    # 固定 shape 为 (3, 2)
    shape = (3, 2)
    # 定义不同的输入类型
    input_data = [[1.1, 2.5, -1.5],[1.1, 2.5, -1.5]]  # 基础数据
    input_types = {
    ...
    for input_name, input_value in input_types.items():
        try:
            # 测试 MindSpore
            ms_output = mint.nn.functional.dropout(input_value)
            # shape 与预期一致
        ...
        try:
            # 测试 PyTorch
            torch_output = torch.nn.functional.dropout(input_value)
            # shape 与预期一致
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_dropout_random_chaotic_input(mode):
    ms.set_context(mode=mode)
    # 定义随机混乱输入
    chaotic_inputs = [
        np.array([[1.1, 2.5, -1.5],[1.1, 2.5, -1.5]], dtype=np.float32),  # np.array 输入
        torch.tensor([[1.1, 2.5, -1.5],[1.1, 2.5, -1.5]], dtype=torch.float32),  # PyTorch tensor 浮点数
        torch.tensor([[1, 2, 3], [4, 5, 6]], dtype=torch.int32),  # PyTorch tensor 整型
        ms.tensor(np.array([[1.1, 2.5, -1.5],[1.1, 2.5, -1.5]]), dtype=ms.float32),  # MindSpore tensor 浮点数
        ms.tensor(np.array([[1, 2, 3], [4, 5, 6]]), dtype=ms.int32),  # MindSpore tensor 整型
        [[1.1, 2.5, -1.5],[1.1, 2.5, -1.5]],  # Python list
        ([1.1, 2.5, -1.5],[1.1, 2.5, -1.5]),  # Python tuple
    ]
    ...
    for input_data in chaotic_inputs:
        try:
            ms_output = mint.nn.functional.dropout(input_data)
        ...
        try:
            torch_output = torch.nn.functional.dropout(input_data)
...
def test_forward_backward(mode):
    ...
    class SimpleModel(nn.Module):
        ...
        def forward(self, x):
            return torch.nn.functional.dropout(self.linear(x))
    ...
    class SimpleModelMS(mnn.Cell):
        ...
        def construct(self, x):
            return mint.nn.functional.dropout(self.linear(x))
