...
def test_ones_different_dtypes(mode):
    """测试不同数据类型下，MindSpore和PyTorch的ones支持度，若支持度相同的场景，比较运算结果是否一致"""
    ...
    for dtype_ms, dtype_torch in zip(dtype_ms_list, dtype_torch_list):
        ...
        try:
            ms_result = mint.ones(size=size, dtype=dtype_ms)
        except Exception as e:
            ms_result = str(e)

        try:
            torch_result = torch.ones(size=size, dtype=dtype_torch)
        except Exception as e:
            torch_result = str(e)
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ones_wrong_input(mode):
    """测试无效输入时，MindSpore和PyTorch的错误处理的异常对比"""
    ms.set_context(mode=mode)

    invalid_inputs = [
        ("invalid", TypeError),  # size 不是整数
        ([1.0, 2.0], TypeError),  # size 不是整数的元组
        (Tensor([1.0, 2.0]), TypeError),  # size 不是整数的Tensor
        ((-1, -2), ValueError)  # size 中包含小于0的
    ]

    for size, expected_exception in invalid_inputs:
        try:
            ms_result = mint.ones(size=size)
        except Exception as e:
            ms_result = str(e)

        # PyTorch
        try:
            torch_result = torch.ones(size=size)
        except Exception as e:
            torch_result = str(e)
