...
def test_gt_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
        ...
        try:
            ms_result = ms.mint.gt(ms_input_1, ms_input_2)
        except Exception as e:
            print(e)
            print(f"mindspore不支持{ms_type}类型")

        try:
            torch_result = torch.gt(torch_input_1, torch_input_2)
        except Exception as e:
            print(e)
            print(f"torch不支持{torch_type}类型")

@pytest.mark.parametrize('mode',[ms.GRAPH_MODE,ms.PYNATIVE_MODE])
def test_gt_input_fixed_dtype(mode):
    ...
    for dtype in all_types:
        ...
        for _ in range(10):
            ...
            if not r:
                result= False
                print(f"当输入类型为{dtype}时，mindspore与torch的gt函数结果不一致，差异位置为{diff}")
                assert result

@pytest.mark.parametrize('mode',[ms.GRAPH_MODE,ms.PYNATIVE_MODE])
def test_gt_input_different_parameters(mode):
...
@pytest.mark.parametrize('mode',[ms.GRAPH_MODE,ms.PYNATIVE_MODE])
def test_gt_input_random_chaotic(mode):
    """测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)
    # 非法数据结构输入
    ...
    for i, input_1 in enumerate(inputs_data_1):
        ...
        try:
            ms_res = ms.mint.gt(ms_input_1, ms_input_2)
            print(ms_res)
        except Exception as e:
            print(e)
        ...
        try:
            torch_res = torch.gt(torch_input_1, torch_input_2)
            print(torch_res)
        except Exception as e:
            print(e)
...
def test_gt_forward_back(mode):
    ...
    def forward_ms(input_1, input_2):
        return ms.mint.gt(input_1, input_2)

    def forward_torch(input_1, input_2):
        return torch.gt(input_1, input_2)
...
if __name__=="__main__":
    test_gt_forward_back(ms.PYNATIVE_MODE)
