...
def is_same(input_data_x=[[1, 2], [3, 4], [5, 6]], input_data_y=[[6, 5], [4, 3], [2, 1]], shape=None, dtype_ms=ms.float32, dtype_torch=torch.float32):
    ...
    ms_result = mint.less(ms_tensor_x, ms_tensor_y).asnumpy()
    torch_result = torch.lt(torch_tensor_x, torch_tensor_y).numpy()
...
def test_less_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
        ...
        try:
            ...
            err = True
            print(f"mint.less not supported for {dtype_ms}")

        try:
            torch_result = torch.lt(torch_tensor_x, torch_tensor_y).numpy()
        except Exception as e:
            err = True
            print(f"torch.lt not supported for {dtype_torch}")

        if not err:
            if not np.allclose(ms_result, torch_result):
                print(f"mint.less is supported for {dtype_ms} but not working properly")


@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_less_random_input_fixed_dtype(mode):
    """测试固定shape，固定输入值，不同输入参数（string\bool等类型），两个框架的支持度"""
    ms.set_context(mode=mode)
    assert True#由于返回值皆为布尔值，因此不再重复测试

@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_less_different_para(mode):
    """测试固定shape，固定输入值，不同输入参数（string\bool等类型），两个框架的支持度"""
    ms.set_context(mode=mode)
    parameters = [None]
    for param in parameters:
        try:
            ms_output = ms.mint.less(ms_tensor_x, ms_tensor_y).asnumpy()
            torch_output = torch.lt(torch_tensor_x, torch_tensor_y, out=param).numpy()
            # 对比输出
...
def test_scalar_comparison(scalar):
    ...
    assert ms.mint.less(ms_tensor, scalar).shape == (3,)

@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_less_wrong_input(mode):
    """测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)
    ...
    # 测试非法输入类型（字符串代替张量）
    try:
        ms.mint.less("invalid_input", valid_tensor)
    ...
    # 测试无效关键字参数
    try:
        ms.mint.less(valid_tensor, valid_tensor)
    except TypeError as e:
...
def test_less_forward_back(mode):
    ...
    def forward_pt(x, y):
        # 将比较结果作为掩码，与可导操作结合
        mask = torch.lt(x, y).float()  # 布尔转浮点
        return (x * mask).sum()  # 可导路径：x * mask

    def forward_ms(x, y):
        mask = mint.less(x, y).astype(ms.float32)  # 使用astype正确转换类型
        return (x * mask).sum()
