...
def generate_data(shape,dtype):#根据shape和类型生成
  if dtype in [np.int8, np.int16, np.int32, np.int64, np.uint8, np.uint16]:
      ...
      input_data = np.abs(np.random.randn(*shape)).astype(dtype)# 由于是开方，所以生成非负数
  return input_data
...
def test_any_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
      ...
      try:
        ms_res=ms.mint.sqrt(ms_input)
        print(ms_res)
      except Exception as e:
        print(f"Mindspore不支持{ms_type}类型")
            
      try:
        torch_res=torch.sqrt(torch_input)
        print(torch_res)
      except Exception as e:
...
def test_any_random_input_fixed_dtype(mode):
    ...
    for type in types:
        ...
        for _ in range(100):
          ...
          ms_res = ms.mint.sqrt(ms_inputs)
          torch_res=torch.sqrt(torch_inputs)
#           print(f"torch_res:\n{torch_res}\nms_res:\n{ms_res}")
          #转为numpy数组进行比较
...
def test_any_different_para(mode):
    ...
    assert True#由于sqrt只有input参数，因此不再重复test_any_different_dtypes的测试
...
def test_any_wrong_input(mode):
    ...
    ms.set_context(mode=mode)
    #4.1特殊数据nan或inf和负数
    torch_input=torch.tensor([float("nan"),float('inf'),float('-inf'),-4]) 
    ms_input=ms.Tensor([float("nan"),float('inf'),float('-inf'),-4]) 
    print("\n特殊数据（负数）nan或inf：")
    torch_res=None
    ms_res=None
    try:
    ...
    for inp in inputs:
      ...
      print("PyTorch：",end="")
      try:
        torch_res=torch.sqrt(inp)
        print(torch_res)  
      except Exception as e:
        print(e)
      print("MindSpore：",end="")
      try:
        ms_res=ms.mint.sqrt(inp)
        print(ms_res)  
      except Exception as e:
        print(e)
      if torch_res is None or ms_res is None:
