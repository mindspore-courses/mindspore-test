...
def test_relu_dtypes_support(mode):
    """测试不同dtype输入的支持情况"""
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_relu_fixed_dtype_accuracy(mode):
    """固定dtype测试输出一致性"""
    ms.set_context(mode=mode, device_target='Ascend')
    ...
    out_ms = F_mint.relu(x_ms)
    
    assert np.allclose(out_ms.asnumpy(), out_torch.detach().numpy(), atol=1e-3), "输出不一致"

@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_relu_invalid_input(mode):
    ms.set_context(mode=mode, device_target='Ascend')
    """测试随机混乱输入，报错信息的准确性"""
    
    # 测试非Tensor输入
    with pytest.raises(TypeError):
        F_mint.relu(np.random.randn(2,3))

class TorchReLUNet(torch.nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        return F_torch.relu(self.linear(x))

class MsReLUNet(ms.nn.Cell):
    def __init__(self):
        super().__init__()
    ...
    def construct(self, x):
        return F_mint.relu(self.linear(x))

@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_relu_network_forward(mode):
    """测试网络正向传播一致性"""
    ms.set_context(mode=mode, device_target='Ascend')
    x_np = np.random.randn(2, 3).astype(np.float32)
    
    # PyTorch
    torch_net = TorchReLUNet()
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_relu_network_backward(mode):
    """测试网络反向传播梯度"""
    ms.set_context(mode=mode, device_target='Ascend')
    x_np = np.random.randn(2, 3).astype(np.float32)
    y_np = np.array([0, 1])
