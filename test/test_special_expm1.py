...
def test_random_tensor():
    ...
    output_pt = torch.special.expm1(input_pt)
    output_ms = ms.mint.special.expm1(input_ms)
...
def test_dype_diff(ms_type, pt_type):
    print(ms_type, pt_type)
    if ms_type == ms.int64 and pt_type == torch.int64:
        input_ms = ms.tensor([[1, -2, 3], [5, -6, 7], [2, 4, 5]], dtype=ms_type)
        output_ms = output_ms = ms.mint.special.expm1(input_ms)
        
        input_pt = torch.tensor([[1, -2, 3], [5, -6, 7], [2, 4, 5]], dtype=pt_type)
        output_pt = torch.special.expm1(input_pt)
...
def test_random_input():
    ...
    reported_flag = False
    try:
        output_ms = ms.mint.special.expm1([1.2, 1.3, 1.4, 1.5])
        print(output_ms)
    except Exception as e:
        reported_flag = True
        e = str(e)
    ...
    if reported_flag == False:
        assert "No error message was reported when the inputs is not tensor" == 1
        
     # 测试输入为空的情况
    reported_flag = False
    try:
        output_ms = ms.mint.special.expm1(None)
        print(output_ms)
    except Exception as e:
        reported_flag = True
        e = str(e)
    ...
    if reported_flag == False:
        assert "No error message was reported when the inputs is None" == 1
...
def test_forward_backward():
    ...
    def ms_func(ms_tensor):
        result_ms = ms.mint.special.expm1(ms_tensor)
        return result_ms.sum()
    
    def pt_func(pt_tensor):
        result_pt = torch.special.expm1(pt_tensor)
