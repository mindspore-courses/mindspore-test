...
def test_any_random_input_fixed_dtype(mode):
    ...
    for type in types:
        for _ in range(100):
          ...
          ms_res = ms.mint.bitwise_xor(ms_inputs,ms_others)
          torch_res=torch.bitwise_xor(torch_inputs,torch_others)
          #转为numpy数组进行比较
...
def test_any_different_para(mode):
    ...
    assert True#由于bitwise_xor只有input、other参数，因此不再重复test_any_different_dtypes的测试
...
def test_any_wrong_input(mode):
    ...
    for i,inp in enumerate(inputs):
      ...
      if type(oth) is np.ndarray:
         ...
         torch_oth=oth
      try:
        torch_res=torch.bitwise_xor(torch_inp,torch_oth)
        print(torch_res)  
      except Exception as e:
        print(e)
      print("MindSpore：",end="")
      ...
      if type(oth) is np.ndarray:
         ...
         ms_oth=oth
      try:
        ms_res=ms.mint.bitwise_xor(ms_inp,ms_oth)
        print(ms_res)  
      except Exception as e:
        print(e)
      assert check(torch_res,ms_res)
...
def test_any_forward_back(mode):
    ...
    def forward_ms(x,y):
        return ms.mint.bitwise_xor(x,y)
