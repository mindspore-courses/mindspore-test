...
def test_eye_different_dtypes(mode):
    """测试不同数据类型下，MindSpore和PyTorch的eye支持度，若支持度相同的场景，比较运算结果是否一致"""
    ...
    for dtype_ms, dtype_torch in zip(dtype_ms_list, dtype_torch_list):
        ...
        try:
            ms_result = mint.eye(m, n, dtype=dtype_ms)
        except Exception as e:
            ms_result = str(e)

        try:
            torch_result = torch.eye(m, n, dtype=dtype_torch)
        except Exception as e:
            torch_result = str(e)
...
def test_eye_wrong_input(mode):
    ...
    for m, n in invalid_inputs:
        # MindSpore
        try:
            ms_result = mint.eye(m, n)
        except Exception as e:
            ms_result = str(e)

        # PyTorch
        try:
            torch_result = torch.eye(m, n)
        except Exception as e:
            torch_result = str(e)
