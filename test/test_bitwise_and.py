...
def test_any_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
      ...
      try:
        ms_res=ms.mint.bitwise_and(ms_input,ms_other)
        print(ms_res)
      ...
      try:
        torch_res=torch.bitwise_and(torch_input,torch_other)
        print(torch_res)
      except Exception as e:
...
def test_any_random_input_fixed_dtype(mode):
    ...
    for type in types:
        for _ in range(100):
          ...
          ms_res = ms.mint.bitwise_and(ms_inputs,ms_others)
          torch_res=torch.bitwise_and(torch_inputs,torch_others)
          #转为numpy数组进行比较
...
def test_any_different_para(mode):
    ...
    assert True#由于bitwise_and只有input、other参数，因此不再重复test_any_different_dtypes的测试
...
def test_any_wrong_input(mode):
    ...
    for i,inp in enumerate(inputs):
      ...
      if type(oth) is np.ndarray:
         ...
         torch_oth=oth
      try:
        torch_res=torch.bitwise_and(torch_inp,torch_oth)
        print(torch_res)  
      except Exception as e:
        print(e)
      print("MindSpore：",end="")
      ...
      if type(oth) is np.ndarray:
         ...
         ms_oth=oth
      try:
        ms_res=ms.mint.bitwise_and(ms_inp,ms_oth)
        print(ms_res)  
      except Exception as e:
        print(e)
...
def test_any_forward_back(mode):
    ...
    def forward_ms(x,y):
        return ms.mint.bitwise_and(x,y)
