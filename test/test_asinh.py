...
def test_asinh_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
        ...
        err = False
        try:
            ms_result = mint.asinh(ms_input).asnumpy()
        except Exception as e:
            err = True
            print(f"mint.asinh not supported for {dtype_ms}")
        try:
            torch_result = torch.asinh(torch_input).numpy()
        except Exception as e:
            err = True
            print(f"torch.asinh not supported for {dtype_torch}")
        if not err:
            try:
...
@pytest.mark.parametrize("mode", [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_asinh_fixed_dtype_random_input(mode):
...
@pytest.mark.parametrize("mode", [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_asinh_fixed_shape_input_diff_param(mode):
    ...
    torch_input = torch.tensor(input_data, dtype=torch.float32)
    try:
        ms_result = mint.asinh(ms_input, "extra_param").asnumpy()
    except Exception as e:
        print(f"mint.asinh extra param error:", e)
    try:
        torch_result = torch.asinh(torch_input, "extra_param").numpy()
    except Exception as e:
        print(f"torch.asinh extra param error:", e)


@pytest.mark.parametrize("mode", [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_asinh_random_input_error_message(mode):
    """1.(d) 测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)
    input_data = np.array([0, -0.5])
    ms_input = Tensor(input_data, ms.float32)
    try:
        ms_result = mint.asinh(1.5).asnumpy()
    except Exception as e:
        print(f"mint.asinh error message:", e)
    try:
        ms_result = mint.asinh(True).asnumpy()
    except Exception as e:
        print(f"mint.asinh error message:", e)
    try:
        ms_result = mint.asinh(ms_input, "extra_param").asnumpy()
