...
def test_random_tensor(mode):
    ...
    input_pt = torch.randn((4, 5), dtype=torch.float32)
    input_ms = ms.tensor(input_pt.numpy(), dtype=ms.float32)
    
    output_pt = torch.expm1(input_pt)
    output_ms = ms.mint.expm1(input_ms)
...
def test_dtype(mode):
    ...
    for dtype in ms_float_lst:
        input_ms = ms.tensor([[1.3, 2.1, 3.7], [2.1, 1.2, 4.2], [4.2, 4.3, 1.3]], dtype=dtype)
        try:
            output_ms = ms.mint.expm1(input_ms)
            print(dtype, output_ms)
        except Exception as e:
    ...
    for dtype in ms_complex_lst:
        input_ms = ms.tensor([[1 + 1j, 1 + 2j, 1 + 3j], [1 + 1.2j, 1 + 2j, 1 + 2.1j]], dtype=dtype)
        try:
            output_ms = ms.mint.expm1(input_ms)
            print(dtype, output_ms)
        except Exception as e:
    ...
    for dtype in pt_float_lst:
        input_pt = torch.tensor([[1.3, 2.1, 3.7], [2.1, 1.2, 4.2], [4.2, 4.3, 1.3]], dtype=dtype)
        try:
            output_pt = torch.expm1(input_pt)
            print(dtype, output_pt)
        except Exception as e:
    ...
    for dtype in pt_complex_lst:
        input_pt = torch.tensor([[1 + 1j, 1 + 2j, 1 + 3j], [1 + 1.2j, 1 + 2j, 1 + 2.1j]], dtype=dtype)
        try:
            output_pt = torch.expm1(input_pt)
            print(dtype, output_pt)
        except Exception as e:
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
@pytest.mark.parametrize('ms_type,pt_type', [[ms.int64, torch.int64], [ms.float16, torch.float16], [ms.float32, torch.float32], [ms.bfloat16, torch.bfloat16], [ms.bool_, torch.bool]])
def test_dype_diff(mode, ms_type, pt_type):
    ms.set_context(mode=mode, device_target='Ascend')
    
    print(ms_type, pt_type)
    if ms_type == ms.int64 and pt_type == torch.int64:
        data_lst = [[1, -2, 3], [5, -6, 7], [2, 4, 5]]
        input_ms = ms.tensor(data_lst, dtype=ms_type)
        output_ms = output_ms = ms.mint.expm1(input_ms)
        
        input_pt = torch.tensor(data_lst, dtype=pt_type)
        output_pt = torch.expm1(input_pt)
        assert np.allclose(output_ms.numpy(), output_pt.numpy(), rtol=1e-3)
    
    if ms_type in ms_float_lst and pt_type in pt_float_lst:
        data_lst = [[1.3, 2.1, 3.7], [2.1, 1.2, 4.2], [4.2, 4.3, 1.3]]
        input_ms = ms.tensor(data_lst, dtype=ms_type)
        output_ms = ms.mint.expm1(input_ms)
...
def test_random_input(mode):
    ...
    reported_flag = False
    try:
        output_ms = ms.mint.expm1([1.2, 1.3, 1.4, 1.5])
        print(output_ms)
    except Exception as e:
        reported_flag = True
        e = str(e)
    ...
    if reported_flag == False:
        assert "No error message was reported when the input is not tensor" == 1
    
    # 测试输入为numpy数组的情况
    reported_flag = False
    try:
        output_ms = ms.mint.expm1(np.array([[1.1, 1.2, 1.3], [1.4, 1.5, 1.6]], dtype=np.float64))
    except Exception as e:
        reported_flag = True
        e = str(e)
...
def test_forward_backward(mode):
    ...
    def ms_func(ms_tensor):
        result_ms = ms.mint.expm1(ms_tensor)
        return result_ms.sum()
    
    def pt_func(pt_tensor):
        result_pt = torch.expm1(pt_tensor)
...
pytest.main(['-vs', 'test_expm1.py', '--html', './report/report.html'])