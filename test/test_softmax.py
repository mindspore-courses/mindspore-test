...
def test_softmax_all_dtypes(mode):
    ...
    for ms_dtype, torch_dtype in zip(ms_dtypes, torch_dtypes):
        ...
        try:
            # 调用 softmax 函数
            ms_output = mint.nn.functional.softmax(ms_input)
            # 对比结果shape
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_softmax_fixed_dtype_random_input(mode):
    ms.set_context(mode=mode)
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_softmax_different_input_types(mode):
    ms.set_context(mode=mode)
    # 固定 shape 为 (3,)
    shape = (3,)
    # 定义不同的输入类型
    ...
    for input_name, input_value in input_types.items():
        try:
            # 测试 MindSpore
            ms_output = mint.nn.functional.softmax(input_value)
            # shape 与预期一致
        ...
        try:
            # 测试 PyTorch
            torch_output = torch.nn.functional.softmax(input_value)
            # shape 与预期一致
...
'''
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_softmax_random_chaotic_input(mode):
    ms.set_context(mode=mode)
    # 定义随机混乱输入
    chaotic_inputs = [
    ...
    for input_data in chaotic_inputs:
        try:
            ms_output = mint.nn.functional.softmax(input_data)
        ...
        try:
            torch_output = torch.nn.functional.softmax(input_data)
...
def test_forward_backward(mode):
    ...
    class SimpleModelMS(mnn.Cell):
        ...
        def construct(self, x):
            return mint.nn.functional.softmax(self.linear(x))
