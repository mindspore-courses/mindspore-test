...
def test_arctan2_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
        ...
        input_data = np.random.uniform(-1e5, 1e5, (2, 3))
        other_data = np.random.uniform(-1e5, 1e5, (2, 3))
        ms_input = Tensor(input_data, dtype_ms)
        ms_other = Tensor(other_data, dtype_ms)
        torch_input = torch.tensor(input_data, dtype=dtype_torch)
        torch_other = torch.tensor(other_data, dtype=dtype_torch)
        err = False
        try:
            ms_result = mint.arctan2(ms_input, ms_other).asnumpy()
        except Exception as e:
            err = True
            print(f"mint.arctan2 not supported for {dtype_ms}")
        try:
            torch_result = torch.arctan2(torch_input, torch_other).numpy()
        except Exception as e:
            err = True
            print(f"torch.arctan2 not supported for {dtype_torch}")
        if not err:
            try:
...
@pytest.mark.parametrize("mode", [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_arctan2_fixed_dtype_random_input(mode):
    """1.(b) 测试固定dtype，random输入值，对比两个框架输出是否相等（误差范围为小于1e-3）"""
    ms.set_context(mode=mode)
    shapes = [(4), (3, 2), (3, 4, 5), (6, 7, 8, 9)]
    for shape in shapes:
        input_data = np.random.uniform(-1e5, 1e5, shape)
        other_data = np.random.uniform(-1e5, 1e5, shape)
...
@pytest.mark.parametrize("mode", [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_arctan2_fixed_shape_input_diff_param(mode):
    ...
    torch_other = torch.tensor(other_data, dtype=torch.float32)
    try:
        ms_result = mint.arctan2(ms_input, ms_other, "extra_param").asnumpy()
    except Exception as e:
        print(f"mint.arctan2 extra param error:", e)
    try:
        torch_result = torch.arctan2(torch_input, torch_other, "extra_param").numpy()
    except Exception as e:
        print(f"torch.arctan2 extra param error:", e)


@pytest.mark.parametrize("mode", [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_arctan2_random_input_error_message(mode):
    """1.(d) 测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)
    input_data = np.array([0, 1])
    other_data = np.array([1])
    ms_input = Tensor(input_data, ms.float32)
    ms_other = Tensor(other_data, ms.float32)
    try:
        ms_result = mint.arctan2(ms_input, ms_other).asnumpy()
    except Exception as e:
        print(f"mint.arctan2 error message:", e)
    try:
        ms_result = mint.arctan2(True).asnumpy()
    except Exception as e:
        print(f"mint.arctan2 error message:", e)
    try:
        ms_result = mint.arctan2(ms_input, ms_other, "extra_param").asnumpy()
...
def test_arctan2_network(mode):
    ...
    torch_other = torch.tensor(other_data, dtype=torch.float32, requires_grad=True)
    ...
    output_pt = forward_pt(torch_input, torch_other)
