...
def is_same(input_data_x=[[1, 2], [3, 4], [5, 6]], input_data_y=[[6, 5], [4, 3], [2, 1]], shape=None, dtype_ms=ms.float32, dtype_torch=torch.float32):
    ...
    ms_result = mint.divide(ms_tensor_x, ms_tensor_y).asnumpy()
    torch_result = torch.divide(torch_tensor_x, torch_tensor_y).numpy()
...
def test_divide_different_dtypes(mode):
    ...
    for i in range(len(ms_dtypes)):
        ...
        try:
            ...
            err = True
            print(f"mint.divide not supported for {dtype_ms}")

        try:
            torch_result = torch.divide(torch_tensor_x, torch_tensor_y).numpy()
        except Exception as e:
            err = True
            print(f"torch.divide not supported for {dtype_torch}")

        if not err:
            if np.allclose(ms_result, torch_result):
                print(f"mint.divide supported for {dtype_ms}")
                print(f"torch.divide supported for {dtype_torch}")
            else:
                print(f"mint.divide is supported for {dtype_ms} but not working properly")


@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_divide_random_input_fixed_dtype(mode):
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_divide_different_para(mode):
    """测试固定shape，固定输入值，不同输入参数（string\bool等类型），两个框架的支持度"""
    ms.set_context(mode=mode)
    parameters = [None, "trunc", "floor"]
    for param in parameters:
        try:
            ms_output = ms.mint.divide(ms_tensor_x, ms_tensor_y,rounding_mode=param).asnumpy()
            torch_output = torch.divide(torch_tensor_x, torch_tensor_y, rounding_mode=param).numpy()
            # 对比输出
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_divide_wrong_input(mode):
    """测试随机混乱输入，报错信息的准确性"""
    ms.set_context(mode=mode)
    try:
        ms_result = mint.divide(ms_tensor_x, 'a').asnumpy()
    except TypeError as e:
        print(f"TypeError - 测试通过！如果 input 和 other 不是以下之一：Tensor、Number、bool。报错信息：\n{e}")

    try:
        ms_result = mint.divide(input_data_x, ms_tensor_y).asnumpy()
    except TypeError as e:
        print(f"TypeError - 测试通过！如果 input 和 other 不是以下之一：Tensor、Number、bool。报错信息：\n{e}")

    try:
        ms_result = mint.divide(ms_tensor_x, ms_tensor_y, rounding_mode="invalid_mode").asnumpy()
    except ValueError as e:
        print(f"ValueError - 测试通过！如果 rounding_mode 不是以下之一： None 、 'floor' 、 'trunc' 。报错信息：\n{e}")


@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_divide_forward_back(mode):
